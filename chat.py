...
NUM_PREVIOUS = 30
STREAM_MAX_LEN = 1000
ALLOWED_ROOMS = ["room:1", "room:2", "room:3"]
PORT = 9080
HOST = "0.0.0.0"
...
async def add_room_user(chat_info: dict, pool):
    ...
    added = await pool.sadd(
        cvar_tenant.get() + ":users", cvar_chat_info.get()["username"]
    )
    return added
...
async def chat_info_vars(username: str = None, room: str = None):
    ...
    :type room:
    """
    if username in ['', 'null', None] and room is None:
        return {"username": str(uuid.uuid4()), "room": ALLOWED_ROOMS[0]}
    return {"username": username, "room": room}


@app.websocket("/ws")
async def websocket_endpoint(
    websocket: WebSocket, chat_info: dict = Depends(chat_info_vars)
):
    # print('request.hostname', websocket.url.hostname)
    tenant_id = ":".join([websocket.url.hostname.replace(".", "_"), chat_info["room"]])
    ...
    if not verified:
        ...
        await asyncio.gather(
            ws_recieve(websocket, chat_info), ws_send(websocket, chat_info)
        )
...
async def get(request: Request):
    return templates.TemplateResponse(
        "chat.html", {"request": request, "ip": get_local_ip(), "port": PORT}
    )
...
async def get(request: Request):
    return templates.TemplateResponse(
        "moderator_chat.html", {"request": request, "ip": get_local_ip(), "port": PORT}
    )
...
async def verify_user_for_room(chat_info):
    ...
    already_exists = await pool.sismember(
        cvar_tenant.get() + ":users", cvar_chat_info.get()["username"]
    )
    if already_exists:
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "chat:app", host=HOST, port=PORT, log_level="info", reload=True
    )  # , uds='uvicorn.sock')
